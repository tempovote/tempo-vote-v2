...
export default function NewGovernanceActionPage({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      setAlert({
        ...
        message: `Transaction đang được xử lý. TX Hash: ${hash.slice(0, 16)}...${hash.slice(-8)}`,
      })
    } catch (e: any) {
      setAlert({
        type: "error",
  ...
  return (
    <main className="page-container py-10 max-w-2xl mx-auto space-y-6">
      ...
      {/* Success: txHash detail below modal (shown alongside modal) */}
      {txHash && (
        <div className="card-static rounded-2xl p-5 space-y-3">
          <p className="text-xs font-semibold text-text-muted uppercase tracking-wider">Transaction Hash</p>
